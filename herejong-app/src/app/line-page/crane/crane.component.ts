...
export class CraneComponent implements OnInit {
  ...
  async initLineLiff() {
    try {
      await this.appService.initLineLiffCustomerCrane();
  ...
  selectType(value: string, id: number) {
    ...
    this.spinner.show();
    this.id = id;
    this.categoryService.getCategorySize(this.id).subscribe(
      data => {
  ...
  onSave() {
    if (this.value == '') {
      Swal.fire({
        icon: "warning",
        title: 'Oops...',
        text: "กรุณาเลือกประเภทเครื่องจักร",
      })
    }
  }
  ...
  onSaveProduct() {
    if (this.valueProduct == '') {
      Swal.fire({
        icon: "warning",
        title: 'Oops...',
        text: "กรุณาเลือกขนาดเครื่องจักร",
      })
    } 
  }
  ...
  openDialogTimeStart(): void {
    const dialogRef = this.dialog.open(DialogTimeStart, {
      width: '98vw',
      maxWidth: '100vw',
  ...
  openDialogNumberDay(): void {
    const dialogRef = this.dialog.open(DialogNumberDay, {
      width: '98vw',
      maxWidth: '100vw',
...
export class DialogMap implements OnInit, AfterViewInit {
...
@Component({
  selector: 'dialog-time-start',
  templateUrl: 'dialog-time-start.html',
  styleUrls: ['./crane.component.css']
})
export class DialogTimeStart implements OnInit {

  timeStartForm: FormGroup;
  submitted = false;
  resultData: DataTimeStart;
  constructor(private formBuilder: FormBuilder,
    public dialogRef: MatDialogRef<DialogTimeStart>,
...
@Component({
  selector: 'dialog-number-day',
  templateUrl: 'dialog-number-day.html',
  styleUrls: ['./crane.component.css']
})
export class DialogNumberDay implements OnInit {

  numberDayForm: FormGroup;
  submitted = false;
  resultData: DataNumberDay;
  constructor(private formBuilder: FormBuilder,
    public dialogRef: MatDialogRef<DialogNumberDay>,
...
@Component({
  ...
  templateUrl: 'dialog-upload-image.html',
  styleUrls: ['./crane.component.css']
})
export class DialogUploadImage implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,
    ...
    @Inject(MAT_DIALOG_DATA) public data: DataUploadImage) {
 ...
 //   this.initLineLiff();
  ...
  openDialogCategorySize(): void {
    const dialogRef = this.dialog.open(DialogCategorySize, {
      width: '95vw',
      maxWidth: '100vw',
...
export class DialogCategory implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<DialogCategory>,
    private categoryService: CategoryService,
    private spinner: NgxSpinnerService,
  ) {
...
@Component({
  selector: 'dialog-category-size',
  templateUrl: 'dialog-category-size.html',
  styleUrls: ['./crane.component.css']
})
export class DialogCategorySize implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<DialogCategorySize>,
    private categoryService: CategoryService,
    private spinner: NgxSpinnerService,
  ...
  ngOnInit(): void {
    ...
    this.spinner.show();
    this.categoryService.getCategorySize(this.resultData.categoryId).subscribe(
      data => {
        this.spinner.hide();
        if (!data) {
          return;
